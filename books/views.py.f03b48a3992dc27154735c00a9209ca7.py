...
class TaxRateListView(RestrictToSelectedOrganizationQuerySetMixin,
    ...
    def get_queryset(self):
        return TaxRate.objects.filter(created_by=self.request.user)
...
class TaxRateCreateView(AutoSetSelectedOrganizationMixin,
    ...
    def form_valid(self, form):
        ...
        return super(TaxRateCreateView, self).form_valid(form)
...
class Print_Bill(AbstractSaleDetailMixin, 
                 generic.DetailView):
    template_name = "books/bill_GST.html"
    model = Invoice
    context_object_name = "bill"

    def get_success_url(self):
        return reverse('bill_gst', args=[self.object.pk])
...
def item_list(request):
    ...
    if user.is_superuser:
        ...
        items_list = Item.objects.filter(created_by=user)
    return render(request, "books/items_list.html", {'items': items_list})
...
def item_create(request):
    ...
    if form.is_valid():
        Item.objects.create(
            ...
            unit_price_excl_tax=form.cleaned_data.get('unit_price_excl_tax'),
            quantity=form.cleaned_data.get('quantity'),
            tax_rate=form.cleaned_data.get('tax_rate'),
            created_by=request.user
        )
        return HttpResponseRedirect(reverse("books:item-list"))
    return render(request, "books/items_create_or_update.html", {'form':form})


class ItemUpdateView(generic.UpdateView):
    fields = ('name', 'description', 'unit_price_excl_tax', 'quantity', 'tax_rate')
    template_name = 'books/items_create_or_update.html'
    success_url = reverse_lazy("books:item-list")
    model = Item
...
def item_delete(request, pk=None):
    ...
    if request.user == item.created_by or request.user.is_superuser:
        item.delete()
        return redirect("books:item-list")
    else:
        return redirect("books:item-list")
...
def item_autocomplete(request):
    ...
    if qs.count() == 0:
        return JsonResponse({'result': [], 'error': 'Items not found'}, status=404)

    q = request.GET.get('q', None)

    if q != "" or q != None:
        qs = qs.filter(name_istartswith=q)
...
@login_required
def item_get_data(request):
    if request.method == 'POST':
        try:
            item_id = int(request.POST.get('id', None))

            item = Item.objects.get(id=item_id)

            itemJSON = serializers.serialize('json', [item, ])
            struct = json.loads(itemJSON)

            itemJSON = struct[0]['fields']

            return JsonResponse(itemJSON)

        except (Item.DoesNotExist, ValueError) as ex:
